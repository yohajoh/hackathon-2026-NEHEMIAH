...
export default function AdminBooksPage() {
  ...
  const physicalBooks: Book[] = booksData?.books?.map((b: Book & { copies?: number; total?: number }) => ({
    ...b,
    total: b.copies ?? b.total,
    type: "physical",
  })) || [];

  const digitalBooksList: Book[] = digitalData?.books?.map((b: Book) => ({
    ...b,
    total: 0,
    type: "digital",
  })) || [];
  ...
  const getDisplayBooks = () => {
    ...
    return source.filter(
      (b) => b.title?.toLowerCase().includes(q) || b.author?.name?.toLowerCase().includes(q) || b.category?.name?.toLowerCase().includes(q),
    );
  };
  ...
  const totalPages = Math.max(1, Math.ceil(activeTab === "categories" ? Math.ceil(categories.length / ITEMS_PER_PAGE) : Math.ceil(displayBooks.length / ITEMS_PER_PAGE)));
    ...
    <>
      <div className="p-6 lg:p-12 space-y-8">
        ...
        <div className="flex items-center gap-6 border-b border-[#E1D2BD]/50">
          {TABS.map((tab) => (
            <button key={tab.key} onClick={() => handleTabChange(tab.key)} className={`pb-3 text-sm font-bold border-b-2 ${activeTab === tab.key ? "text-[#2B1A10] border-[#2B1A10]" : "text-[#AE9E85] border-transparent"}`}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) setShowCategoryModal(false); }}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between px-8 pt-7 pb-4 border-b border-[#E1D2BD]/50">
              <h3 className="text-xl font-serif font-extrabold text-[#2B1A10]">{editingCategoryId ? "Edit Category" : "Add Category"}</h3>
              <button onClick={() => setShowCategoryModal(false)} className="w-8 h-8 flex items-center justify-center text-[#AE9E85] hover:text-[#2B1A10] rounded-lg"><X size={18} /></button>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); handleSaveCategory(); }} className="px-8 py-6 space-y-4">
              <div><label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Category Name</label><input type="text" required value={categoryName} onChange={(e) => setCategoryName(e.target.value)} className="w-full px-3 py-2.5 text-sm border border-[#E1D2BD] rounded-xl text-[#2B1A10]" /></div>
              <button type="submit" disabled={createCategoryFn.isPending || updateCategoryFn.isPending} className="w-full py-3 bg-[#2B1A10] text-white text-sm font-bold rounded-xl disabled:opacity-50">
                {createCategoryFn.isPending || updateCategoryFn.isPending ? "Saving..." : editingCategoryId ? "Update Category" : "Create Category"}
              </button>
            </form>
          </div>
        </div>
      ...
      {showConditionModal && <ConditionModal show={showConditionModal} onClose={() => { setShowConditionModal(false); setConditionBookId(""); setConditionBookTitle(""); }} bookId={conditionBookId} title={conditionBookTitle} />}
    </>
  );
}
...
function BookTable({ books, onDelete, onEdit, deletingId, onCondition }: { books: Book[]; onDelete: (id: string, type: "physical" | "digital") => void; onEdit: (book: Book) => void; deletingId?: string; onCondition: (id: string, title: string) => void }) {
  ...
  return (
    <>
      ...
      {books.map((book) => (
        <div key={book.id} className="grid grid-cols-[2.5fr_2fr_1.5fr_1fr_1fr_auto] gap-4 items-center px-6 py-4 border-b border-[#E1D2BD]/30 hover:bg-[#FDFAF6]">
          ...
          <span className="text-sm text-[#2B1A10]/70 text-center">{book.type === "digital" ? "Digital" : (book.total ?? "—")}</span>
          <span className={`text-xs font-bold px-2.5 py-1 rounded-lg w-fit ${book.type === "digital" ? "bg-[#F3EFE6] text-[#2B1A10]" : book.available === 0 || book.status === "BORROWED" ? "bg-red-50 text-red-600" : "bg-green-50 text-green-700"}`}>
            {book.type === "digital" ? (book.pdf_access === "RESTRICTED" ? "Read Only" : "Download Allowed") : (book.status || (book.available === 0 ? "Out of Stock" : "Available"))}
          </span>
...
function CategoryTable({ categories, onEdit, onDelete, deletingId }: { categories: Category[]; onEdit: (category: Category) => void; onDelete: (id: string) => void; deletingId?: string }) {
  if (categories.length === 0) return <div className="py-16 text-center text-sm text-[#AE9E85]">No categories found</div>;
  return (
    <>
      ...
      {categories.map((cat) => (
        <div key={cat.id} className="grid grid-cols-[3fr_1fr_auto] gap-4 items-center px-6 py-4 border-b border-[#E1D2BD]/30 hover:bg-[#FDFAF6]">
          <span className="text-sm font-bold text-[#2B1A10]">{cat.name}</span>
          <span className="text-sm text-[#2B1A10]/70">{(cat._count?.books || 0) + (cat._count?.digital_books || 0)}</span>
          <div className="flex items-center gap-2">
            <button onClick={() => onEdit(cat)} className="w-8 h-8 flex items-center justify-center rounded-lg text-[#AE9E85] hover:text-[#2B1A10] hover:bg-[#F3EFE6]"><Pencil size={15} /></button>
            <button onClick={() => onDelete(cat.id)} disabled={deletingId === cat.id} className="w-8 h-8 flex items-center justify-center rounded-lg text-[#AE9E85] hover:text-red-500 hover:bg-red-50 disabled:opacity-40"><Trash2 size={15} /></button>
          </div>
        </div>
      ))}
    </>
...
function AddBookModal({ show, onClose, authors, categories, editingBook, onSubmit, submitting }: { show: boolean; onClose: () => void; authors: Author[]; categories: Category[]; editingBook?: Book | null; onSubmit: (type: "physical" | "digital", data: FormData) => Promise<void>; submitting: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setForm({ title: "", author_id: "", category_id: "", copies: "", pages: "", description: "", publication_year: "", tags: "", topics: "", pdf_access: "RESTRICTED" });
    setImageFile(null); setGalleryFiles([]); setPdfFile(null);
  };
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="px-8 py-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div><label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Title *</label><input type="text" value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} required className="w-full px-3 py-2.5 text-sm border border-[#E1D2BD] rounded-xl text-[#2B1A10]" /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Author *</label><select value={form.author_id} onChange={(e) => setForm((f) => ({ ...f, author_id: e.target.value }))} required className="w-full px-3 py-2.5 text-sm border border-[#E1D2BD] rounded-xl text-[#2B1A10]"><option value="">Select author</option>{authors.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select></div>
            <div><label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Category *</label><select value={form.category_id} onChange={(e) => setForm((f) => ({ ...f, category_id: e.target.value }))} required className="w-full px-3 py-2.5 text-sm border border-[#E1D2BD] rounded-xl text-[#2B1A10]"><option value="">Select category</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div><label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Pages</label><input type="number" min="1" value={form.pages} onChange={(e) => setForm((f) => ({ ...f, pages: e.target.value }))} placeholder="e.g. 300" className="w-full px-3 py-2.5 text-sm border border-[#E1D2BD] rounded-xl text-[#2B1A10]" /></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Tags</label><input type="text" value={form.tags} onChange={(e) => setForm((f) => ({ ...f, tags: e.target.value }))} placeholder="comma separated" className="w-full px-3 py-2.5 text-sm border border-[#E1D2BD] rounded-xl text-[#2B1A10]" /></div>
            <div><label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Topics</label><input type="text" value={form.topics} onChange={(e) => setForm((f) => ({ ...f, topics: e.target.value }))} placeholder="comma separated" className="w-full px-3 py-2.5 text-sm border border-[#E1D2BD] rounded-xl text-[#2B1A10]" /></div>
          </div>
          <div><label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Description</label><textarea rows={3} value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} className="w-full px-3 py-2.5 text-sm border border-[#E1D2BD] rounded-xl text-[#2B1A10] resize-none" /></div>
          {modalTab === "digital" && <div><label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Access Mode</label><select value={form.pdf_access} onChange={(e) => setForm((f) => ({ ...f, pdf_access: e.target.value as "FREE" | "PAID" | "RESTRICTED" }))} className="w-full px-3 py-2.5 text-sm border border-[#E1D2BD] rounded-xl text-[#2B1A10]"><option value="RESTRICTED">Read Only (Restricted)</option><option value="FREE">Free Download</option><option value="PAID">Paid</option></select></div>}
          <div>
            <label className="block text-sm font-bold text-[#2B1A10] mb-1.5">Cover Image</label>
            <div onClick={() => imageInputRef.current?.click()} className="w-full h-24 border-2 border-dashed border-[#E1D2BD] rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-[#8B6B4A]">
              {imageFile ? <p className="text-xs text-[#AE9E85]">{imageFile.name}</p> : <><Upload size={20} className="text-[#AE9E85]" /><p className="text-xs text-[#AE9E85]">Click to upload</p></>}
            </div>
          ...
          <div>
            ...
            <div onClick={() => galleryInputRef.current?.click()} className="w-full h-20 border-2 border-dashed border-[#E1D2BD] rounded-2xl flex items-center justify-center cursor-pointer hover:border-[#8B6B4A]">
              <p className="text-xs text-[#AE9E85]">{galleryFiles.length > 0 ? `${galleryFiles.length} image(s) selected` : "Click to select multiple images"}</p>
            </div>
...
function ConditionModal({ show, onClose, bookId, title }: { show: boolean; onClose: () => void; bookId: string; title: string }) {
  ...
  const getConditionColor = (condition: string) => {
    switch (condition) {
      ...
      case "WORN": return "bg-yellow-100 text-yellow-700 border-yellow-200";
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl overflow-hidden">
        <div className="flex items-center justify-between px-8 pt-7 pb-4 border-b border-[#E1D2BD]/50">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center text-[#AE9E85] hover:text-[#2B1A10] rounded-lg"><X size={18} /></button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 min-h-[500px]">
          ...
          <div className="border-r border-[#E1D2BD]/50 p-6 bg-[#FDFAF6]">
            ...
            {copiesLoading ? (
              <div className="flex items-center justify-center py-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#8B6B4A]"></div></div>
            ) : (
                ...
                {copies.length === 0 ? (
                  ...
                  copies.map((copy) => (
                    <button
                      key={copy.id}
                      type="button"
                      onClick={() => { setSelectedCopyId(copy.id); setConditionForm({ condition: copy.condition, notes: copy.notes || "" }); }}
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${getConditionColor(copy.condition)}`}>{copy.condition}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-[#AE9E85]">{copy.is_available ? "Available" : "Checked Out"}</span>
                        <span className="text-xs text-[#AE9E85]">{new Date(copy.last_condition_update).toLocaleDateString()}</span>
                      </div>
                    </button>
                  ))
                )}
            ...
            {!selectedCopyId ? (
              ...
              <div className="space-y-4">
                <div className="bg-[#FDFAF6] rounded-2xl p-4 border border-[#E1D2BD]/50">
                  ...
                  <div className="text-xs text-[#AE9E85] mt-1">Current: <span className={`font-bold ${getConditionColor(selectedCopy?.condition || "")}`}>{selectedCopy?.condition}</span></div>
                </div>
                ...
                <button
                  ...
                  disabled={!selectedCopyId || updateCondition.isPending || conditionForm.condition === selectedCopy?.condition}
                ...
                {conditionForm.condition !== selectedCopy?.condition && (
                  <div className="text-xs text-center text-[#8B6B4A]">
                    Change: <span className="line-through">{selectedCopy?.condition}</span> → <span className="font-bold">{conditionForm.condition}</span>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="p-6 bg-[#FAFAFA]">
            ...
            {historyLoading ? (
              ...
              <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
                {history.length === 0 ? (
                  <div className="text-sm text-[#AE9E85] text-center py-8">No history yet.<br/>Update a copy to see history.</div>
                ) : (
                  history.map((entry) => (
                    <div key={entry.id} className="rounded-2xl border border-[#E1D2BD] px-4 py-3 bg-white">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <span className={`text-xs font-bold px-2 py-0.5 rounded ${getConditionColor(entry.old_condition)}`}>{entry.old_condition}</span>
                          <span className="text-[#AE9E85]">→</span>
                          <span className={`text-xs font-bold px-2 py-0.5 rounded ${getConditionColor(entry.new_condition)}`}>{entry.new_condition}</span>
                        </div>
                      </div>
                      <div className="text-xs text-[#AE9E85]">{new Date(entry.created_at).toLocaleString()}</div>
                      {entry.notes && <div className="text-xs text-[#2B1A10]/70 mt-2 pt-2 border-t border-[#E1D2BD]/50">{entry.notes}</div>}
                    </div>
                  ))
                )}
              </div>
