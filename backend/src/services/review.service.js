...
export const createReview = async ({ userId, bookType, bookId, rating, comment }) => {
  ...
  if (existing) {
    throw new AppError(
      'You have already reviewed this book. You can edit your existing review.',
      409
    );
  }
...
export const getAllReviews = async (query) => {
  ...
  return {
    ...
    stats: {
      ...
      averageRating: avgStats._avg.rating
        ? parseFloat(Number(avgStats._avg.rating).toFixed(2))
        : 0,
    },
    meta: paginationMeta(total, page, limit),
  };
