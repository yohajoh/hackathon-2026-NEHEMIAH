-- Restore delegated identity + actor audit model after rollback migrations
...
-- If an admin account also has student data, grant student persona.
INSERT INTO "UserRoleAssignment" ("id", "user_id", "role", "created_at")
SELECT gen_random_uuid(), u."id", 'STUDENT'::"Role", CURRENT_TIMESTAMP
FROM "User" u
WHERE (u."role" = 'ADMIN' OR u."student_id" IS NOT NULL)
  AND (u."student_id" IS NOT NULL OR u."year" IS NOT NULL OR u."department" IS NOT NULL)
  AND NOT EXISTS (
    SELECT 1 FROM "UserRoleAssignment" ura
    WHERE ura."user_id" = u."id" AND ura."role" = 'STUDENT'::"Role"
  );

-- Create student profile for student-capable users.
...
-- Backfill actor as current user for existing records.
...
-- Backfill student profile ownership for existing rows.
UPDATE "Rental" r
